...
*/
bool
XmlReader::HasAttr(const String& name) const
{
    n_assert(this->IsOpen());
    n_assert(0 != this->curNode);
    n_assert(name.IsValid());
    return (0 != this->curNode->Attribute(name.AsCharPtr()));
}
...
*/
String
XmlReader::GetString(const String& name) const
{
    n_assert(this->IsOpen());
    n_assert(0 != this->curNode);
    n_assert(name.IsValid());
    String str;
    const char* val = this->curNode->Attribute(name.AsCharPtr());    
    if (0 == val)
    {
        n_error("XmlReader: attribute '%s' doesn't exist on node '%s'!", name.AsCharPtr(), this->curNode->Value());
    }
    else
    {
        str = val;
...
*/
bool
XmlReader::GetBool(const String& name) const
{
...
*/
int
XmlReader::GetInt(const String& name) const
{
...
*/
float4
XmlReader::GetFloat4(const String& name) const
{
#if __NEBULA2_EXPORT__
...
*/
matrix44
XmlReader::GetMatrix44(const String& name) const
{
...
*/
String
XmlReader::GetOptString(const String& name, const String& defaultValue) const
{
...
*/
bool
XmlReader::GetOptBool(const String& name, bool defaultValue) const
{
    if (this->HasAttr(name))
    {
...
*/
int
XmlReader::GetOptInt(const String& name, int defaultValue) const
{
    if (this->HasAttr(name))
    {
...
*/
float
XmlReader::GetOptFloat(const String& name, float defaultValue) const
{
    if (this->HasAttr(name))
    {
...
*/
float4
XmlReader::GetOptFloat4(const String& name, const float4& defaultValue) const
{
    if (this->HasAttr(name))
    {
...
*/
matrix44
XmlReader::GetOptMatrix44(const String& name, const matrix44& defaultValue) const
{
    if (this->HasAttr(name))
    {
